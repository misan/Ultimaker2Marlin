...
int ticks_full_speed;
int ticks_half_speed;
int ticks_low_speed;
int current_layer = 0;
float layer_z_height=0;

void startNewLayer () 
	{
#ifdef DEBUG_INFO
	SERIAL_ECHO_START ;
	SERIAL_ECHOLNPGM ("Layer completed -- speed report: ");
	unsigned long total_ticks = ticks_full_speed+ ticks_half_speed+ticks_low_speed;
	if (total_ticks< 1) total_ticks = 1;
	SERIAL_ECHOPGM ( " Percent time at half speed ");
	SERIAL_ECHO (100 * ticks_half_speed / total_ticks);
	SERIAL_ECHOPGM ( "%\t and at low speed ");
	SERIAL_ECHO (100 * ticks_low_speed / total_ticks);
	SERIAL_ECHOLNPGM(("%"));
#endif
	...
	ticks_low_speed=0;
	current_layer ++;
	layer_z_height = current_position[Z_AXIS];
	SERIAL_ECHO_START ;
	SERIAL_ECHOPGM(("Starting layer #"));
	SERIAL_ECHO (current_layer);
	SERIAL_ECHOPGM(("\t out of "));
	SERIAL_ECHO (total_layers);
	SERIAL_ECHOPGM (" \t at Z height of ");
	SERIAL_ECHO (layer_z_height);
	SERIAL_ECHOLN ("");
	}
//--------------------------------------------------------------------------------------
...
// 			strcpy (card.longFilename,card.filename);
        ...
        for (uint8_t n = 0; n < LINES_IN_ULTICODE_HEADER; n++)
        {
            ...
            { 
				...
				total_layers = LCD_CACHE_DETAIL.total_layers;
				}
...
void lcd_menu_print_heatup()
{
    ...
    *c++ = 0;
    ...
    if (draw_filename)
    {
        ...
        lcd_lib_invert(3, HALF_ROW(ROW8) - 1, progress, DISPLAY_BOTTOM);
    }
...
// return DONTSKIP;
 ...
 //   if (current_block == NULL || moves < 1) { analogWrite(LED_PIN, 0); return SKIP ; }
    if (moves < 2) { slow_buffer_counter += 1; lcd_lib_led_color(255, 0, 0); lcd_lib_update_RGB_LED();ticks_low_speed++;}
    else
        if (moves < BLOCK_BUFFER_SIZE / 4) { slow_buffer_counter += 2; lcd_lib_led_color(255, 0, 160); lcd_lib_update_RGB_LED();ticks_half_speed++;}
        else
            if (moves < BLOCK_BUFFER_SIZE / 2) {  slow_buffer_counter += 4;  lcd_lib_led_color(192, 32, 192); lcd_lib_update_RGB_LED();}
			else { ticks_full_speed++; return DONTSKIP;} 
        ...
        default:
            {
                ...
                *c++ = 0;
				...
				c= buffer+3;
				c = int_to_string(last_head_temp, c, PSTR( DEGREE_C_SYMBOL ));
				*c++ = 0;
				lcd_lib_draw_string(5, ROW6, buffer);

				strcpy_P(buffer, PSTR ("STP:  "));
				c= buffer+4;
				c = int_to_string(last_step_temp, c, PSTR( DEGREE_C_SYMBOL ));
				*c++ = 0;
				lcd_lib_draw_string_center(ROW6, buffer);

				strcpy_P(buffer, PSTR ("MB:  "));
				c= buffer+3;
				c = int_to_string(last_mobo_temp, c, PSTR( DEGREE_C_SYMBOL ));
				*c++ = 0;
...
//                 *c++ = TEMPERATURE_SEPARATOR;
    ...
    if (draw_filename)
    {
        ...
        lcd_lib_draw_hline (3, 125, ROW8 - 1);
        lcd_lib_draw_vline(3, ROW8, DISPLAY_BOTTOM);
        lcd_lib_draw_vline(125, ROW8, DISPLAY_BOTTOM);
...
#pragma GCC diagnostic ignored "-Wmaybe-uninitialized" // Code that causes warning goes here 
        ...
        lcd_lib_draw_string(5, ROW7, buffer);
...
#pragma GCC diagnostic pop
        ...
        lcd_lib_draw_string_right(ROW7, buffer);

// draw current layer number
		c = buffer;
		c = int_to_string(current_layer, c);
		*c++ = '/';
		c = int_to_string(total_layers, c);
				*c++ = 0;	
        ...
        if (!lcd_lib_show_message (ROW8 + 1))
        {
            if (time_phase0)
                lcd_lib_draw_string_center(ROW8 + 1, last_print_name);
            else
            {
                // show the Z height
                buffer[0] = 'Z';
				...
				if (!retracted && layer_z_height!=current_position[Z_AXIS])		// won't handle z-lift implemented in gcode....
					startNewLayer();
                *c++ = 0;
                lcd_lib_draw_string_center(ROW8 + 1, buffer);

            }
            lcd_lib_invert(3, ROW8 - 1, progress, 63);
        }
    }
    lcd_lib_update_screen();
}
...
#if EXTRUDERS > 1
        case 3:
			strcpy_P(c, PSTR("Extr change restract"));
			break;
#endif
        default:
            strcpy_P(c, PSTR("???"));

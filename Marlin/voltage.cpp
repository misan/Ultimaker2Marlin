...
// reads the voltage divider connected to an analog pin, measuring the PSU voltage (24 v nominal)
float readVoltage ()
{
#if MAIN_VOLTAGE_MEASURE_PIN>-1
// read 4 times and take average
    float v = analogRead(MAIN_VOLTAGE_MEASURE_PIN);
	 v += analogRead(MAIN_VOLTAGE_MEASURE_PIN);
	 v += analogRead(MAIN_VOLTAGE_MEASURE_PIN);
	 v += analogRead(MAIN_VOLTAGE_MEASURE_PIN);
    v = v  / (1024*4);
    v *= VCC;
...
float readAVR_VCC()		// we have an external Zener reference of 4.2volts connected to an analog pin
{
	...
	v_ref = analogRead(EXT_VOLTAGE_REF_PIN);
	v_ref += analogRead(EXT_VOLTAGE_REF_PIN);
	v_ref += analogRead(EXT_VOLTAGE_REF_PIN);
	v_ref += analogRead(EXT_VOLTAGE_REF_PIN);
	v_ref /=4.0;
	temp_irq_phase = 8;
#ifdef DEBUG
	SERIAL_ECHOPAIR("Voltage reference RAW =", v_ref);
#endif 
    ...
    if (result < MIN_VOLTAGE ||result > MAX_VOLTAGE ) 
		{ 
#ifdef DEBUG
		SERIAL_ECHOPAIR("\t Rejected reading as out of range = ",result);

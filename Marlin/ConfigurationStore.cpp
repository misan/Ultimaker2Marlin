...
void Config_PrintSettings()
{
    ...
    SERIAL_ECHO_START;
    SERIAL_ECHOLNPGM("Advanced variables: S=Min feed (mm/s), T=Min travel feed (mm/s), B=min segment time (ms), X=max XY jerk (mm/s),  Z=max Z jerk (mm/s),  E=max E jerk (mm/s)");
    SERIAL_ECHO_START;
    ...
    SERIAL_ECHOLNPGM("Extruders:");
    SERIAL_ECHO_START;
    SERIAL_ECHOPAIR(" #1 max temp = " ,(unsigned long ) HEATER_0_MAXTEMP );
    SERIAL_ECHOLN("");
#if (EXTRUDERS > 1 	)
#ifdef HEATER_1_MAXTEMP
    SERIAL_ECHOPAIR(" #2 max temp = " ,HEATER_1_MAXTEMP );
#endif
#if (EXTRUDERS > 2 	)
#ifdef HEATER_2_MAXTEMP
    SERIAL_ECHOPAIR(" #3 max temp = " ,HEATER_2_MAXTEMP );
#endif
#endif
#endif
    SERIAL_ECHOPAIR(" Bed max temp = " ,(unsigned long )  BED_MAXTEMP );
    SERIAL_ECHOLN("");

	SERIAL_ECHOLNPGM("Maximum Positions (mm):");
    SERIAL_ECHO_START;
    SERIAL_ECHOPAIR(" X" ,max_pos[0] );
    SERIAL_ECHOPAIR(" Y" ,max_pos[1] );
    SERIAL_ECHOPAIR(" Z" ,max_pos[2] );
    SERIAL_ECHOLN("");

    SERIAL_ECHO_START;
    SERIAL_ECHOLNPGM("Retraction:");
    SERIAL_ECHO_START;
    SERIAL_ECHOPAIR("  M209 S",retract_length );
    ...
    if (strncmp(ver,stored_ver,3) == 0)
        {
            ...
            X_MAX_LENGTH = max_pos[X_AXIS] - X_MIN_POS;
            Y_MAX_LENGTH = max_pos[Y_AXIS] - Y_MIN_POS;

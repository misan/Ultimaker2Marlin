...
char* drawStatsInfo( char offset, char * buffer, char* c );
//-----------------------------------------------------------------------------------------------------------------
void clearHistory();
//-----------------------------------------------------------------------------------------------------------------
void lcd_init()
...
// 	SERIAL_ECHOLNPGM("MENU MAIN");
	...
	lcd_triple_menu_low(PSTR("PRINT"), PSTR("MAT"), PSTR("SYSTEM"));
	...
	lcd_lib_draw_string(5,HALF_ROW(ROW1), buffer);
	c = buffer;
	...
	lcd_lib_draw_string(64+12,HALF_ROW(ROW1), buffer);
	c = buffer;
	lcd_lib_draw_hline(0,127,HALF_ROW(ROW2)-1);

	// we haven't printed anything, so cycle through a set of information screens
	bool did_print =(starttime !=0 && stoptime !=0 ) ;

	if (!lcd_lib_show_message (HALF_ROW(ROW3)))
		{
		...
		if (IS_SELECTED_MAIN(MAIN_MENU_FILAMENT) || ((time_phase_a(2) && (!recently_printed))))	// SHOW CURRENTLY LOADED MATERIAL SETTINGS (EXTR 0)
			{
			c = drawMaterialInfoScreen(HALF_ROW(0),c, buffer);
			lcd_lib_update_screen();
			return;
			}
		...
		if (IS_SELECTED_MAIN(MAIN_MENU_SYSTEM) || ((time_phase_b(2) && (!recently_printed)	)))	// SHOW SYSTEM INFO (volts, uptime)
			{
			c = drawSystemInfoScreen(HALF_ROW(0), c, buffer);
			lcd_lib_update_screen();
			return;
			}
		if (IS_SELECTED_MAIN(MAIN_MENU_PRINT)|| (time_phase_c(2) || recently_printed)	)	// SHOWLAST PRINT INFO, OR SYSTEM VERSION INFO
			{
			if (did_print)
				c = drawLastPrintInfo(HALF_ROW(0), buffer, c);
			else
				drawCustomFirmwareInfo(HALF_ROW(0));
			lcd_lib_update_screen();
			return;
			}
		//  if (time_phase_d && !recently_printed)			// SHOW LIFETIME STATS						// default case,  so at least we aleways draw *something*
			{
			c = drawStatsInfo(HALF_ROW(0),buffer, c);
			lcd_lib_update_screen();
			return;
			}
		}
...
char* drawSystemInfoScreen( char offset, char* c, char * buffer )
	...
	*c++=0;
	...
	if (!DHT11_disable)
		...
		c = int_to_string(last_mobo_temp_reading, c,  PSTR( DEGREE_C_SYMBOL));
		*c++=0;
		lcd_lib_draw_string(5,offset+ROW4, buffer);
		c = buffer+6;
		strcpy_P(buffer, PSTR("HUMID:"));
		c = int_to_string(last_humid, c,  PSTR( "%"));
		lcd_lib_draw_string_right (offset+ROW4, buffer,120);
		}
	else 
#endif
		{
		if (digitalRead(HEAD_FAN_PIN)) 
			lcd_lib_draw_stringP(5,offset+ROW4, PSTR ("HD FAN ON"));
		else 
			lcd_lib_draw_stringP(5,offset+ROW4, PSTR ("HD FAN OFF"));

		if (digitalRead(MOTHERBOARD_FAN)) 
			lcd_lib_draw_string_rightP(offset+ROW4, PSTR ("MB FAN ON"));
		else 
			lcd_lib_draw_string_rightP(offset+ROW4, PSTR ("MB FAN OFF"));
	...
	*c++=0;
	lcd_lib_draw_string_center(offset+ROW5, buffer);

	strcpy_P(buffer, PSTR ("HD:  "));
	c= buffer+3;
	c = int_to_string((int) last_head_temp, c, PSTR( DEGREE_C_SYMBOL ));
	*c++ = 0;
	lcd_lib_draw_string(5, offset+ROW6, buffer);

	strcpy_P(buffer, PSTR ("STP:  "));
	c= buffer+4;
	c = int_to_string((int) last_step_temp, c, PSTR( DEGREE_C_SYMBOL ));
	*c++ = 0;
	lcd_lib_draw_string_center(offset+ROW6, buffer);

	strcpy_P(buffer, PSTR ("MB:  "));
	c= buffer+3;
	c = int_to_string((int) last_mobo_temp, c, PSTR( DEGREE_C_SYMBOL ));
	*c++ = 0;
	lcd_lib_draw_string_right(offset+ROW6, buffer);
...
//-----------------------------------------------------------------------------------------------------------------
char* drawLastPrintInfo( char offset,char * buffer, char* c )
	{
	unsigned long printTimeSec = (stoptime-starttime)/1000;
	if (printTimeSec < 3) return c;
	lcd_lib_draw_string_center(ROW2+offset, last_print_name);
	...
	*c++=0;
	lcd_lib_draw_string_center(ROW4+offset, buffer);
	c = buffer;
	if (last_print_aborted)
		{ 
		strcpy_P(buffer, PSTR("ABORTED"));
		lcd_lib_draw_string_center(ROW5+offset, buffer);
	...
	lcd_lib_draw_string_center(ROW6+offset, buffer);
	return c;
	}
...
//-----------------------------------------------------------------------------------------------------------------
void drawCustomFirmwareInfo(char offset)
	{
	lcd_lib_draw_string_centerP(ROW2+offset,PSTR ("UM" SQUARED_SYMBOL " - Nerd fork"));
	if (time_phase0)
		lcd_lib_draw_string_centerP((offset+ROW3),PSTR (STRING_CONFIG_H_AUTHOR));
	else
		lcd_lib_draw_string_centerP((offset+ROW4),PSTR (STRING_VERSION_CONFIG_H));
	if (time_phase0)
		lcd_lib_draw_string_centerP(offset+HALF_ROW(ROW5),PSTR ("USE_AT_YOUR_OWN_RISK!"));
	}
...
char* drawStatsInfo( char offset,char * buffer, char* c )
	...
	*c++=0;
	...
	lcd_lib_draw_string_center(ROW5+offset, buffer);	return c;
	}
